...
class EventSchema(ma.Schema):
    name = fields.String(required=True, validate=validate.Length(min=3, error="Event name must be at least 3 characters long."))
    description = fields.String(validate=validate.Length(max=255, error="Description cannot exceed 255 characters."))
    date = fields.Date(required=True, validate=validate.Range(min=datetime.today().date(), error="Event date must be today or in the future."))
    ...
    class Meta:
        fields = ("event_id", "name", "description", "date", "time", "organiser", "venue", "event_participants")
